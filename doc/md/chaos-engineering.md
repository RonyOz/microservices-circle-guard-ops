...
Para un alcance de 3 experimentos, Chaos Mesh minimiza superficie y deuda: cada experimento es un YAML versionado y auto-reversible.
...
- Dashboard incluido: `kubectl port-forward -n chaos-mesh svc/chaos-dashboard 2333:2333`.
...
| **Resultado** | **PASSED.** Pod víctima `promotion-service-778d5f4848-ghd88` eliminado; el Deployment completó el rollout y quedó `Running` con `promotion-service-778d5f4848-4blgp`. Recuperación dentro del timeout (<3m), por debajo del `for: 2m` de `ServiceDown` — la alerta no debió dispararse. |
...
| **Resultado** | **PASSED.** Las 10 llamadas respondieron HTTP 200 (sin cascada de 5xx) con el fallback degradado `{"error":"Service unavailable"}`; el breaker `promotionService` alcanzó estado OPEN dentro de la ventana de scrape. Al expirar el `duration`, el breaker recuperó vía HALF_OPEN → CLOSED. |

El camino HTTP protegido por el breaker es `dashboard → promotion`. El otro borde de promotion (`promotion → notification`) es **Kafka**, que ya tolera la indisponibilidad del consumidor — los mensajes esperan en el topic, no hay circuit breaker que validar ahí. Por eso la partición se diseñó sobre el borde HTTP.

## Experimento 3 — Resource stress (`chaos/experiments/stress-promotion.yaml`)

| | |
|---|---|
| **Hipótesis** | Bajo presión de CPU dentro del pod, el consumo sube de forma observable (Grafana/Prometheus) pero el límite de CPU del chart contiene el impacto: ni el nodo ni los demás servicios se degradan. |
| **Inyección** | `StressChaos` CPU `workers: 2, load: 80`, `mode: one`, `duration: 5m`. |
| **Verificación automática** | Baseline de `system_cpu_usage{app="promotion-service"}`, inyección, espera 90s y **falla si el valor no se multiplica ≥2× y supera 0.10**. |
...
| **Resultado** | **PASSED.** `system_cpu_usage` de promotion-service subió de **0.039 a 0.997** durante el stress. El límite de CPU del pod contuvo el blast radius: ningún otro servicio ni el nodo se degradaron. Stress auto-revertido a los 5m. |
...
4. `./scripts/scale-down.sh`

## Aprendizajes integrados en la arquitectura

- **Timeouts en clientes HTTP.** Al diseñar el Experimento 2 se detectó que `PromotionClient` e `IdentityClient` no definían timeouts: bajo partición las llamadas colgarían indefinidamente y el breaker **nunca** habría abierto. Se añadieron `connectTimeout=2s` / `readTimeout=3s` — los timeouts son lo que convierte una caída en fallos contables para el breaker. Es exactamente el hueco que el chaos engineering existe para exponer, y quedó integrado en la arquitectura antes de ejecutar las pruebas.

Con ese ajuste en su lugar, los tres experimentos confirmaron las hipótesis en su primera ejecución; no se requirieron correcciones posteriores.
